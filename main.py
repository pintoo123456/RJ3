...
def welcome_plan(message):
    ...
    response = f'''{user_name}, ʙᴜʏ ғʀᴏᴍ @RJhacker111
...
def attack(message):
    ...
    if user_id not in AUTHORIZED_USERS:
        bot.send_message(message.chat.id, "ऑनर से बात करो :- @RJhacker111")
        return
    try:
...
def stop(message):
    ...
    if user_id not in AUTHORIZED_USERS:
        bot.send_message(message.chat.id, "🚫 Access Denied! Contact the owner for assistance: @RJhacker111")
        return
